...
import type { BuyingGuide } from "@/lib/content";
...
interface GuideModalProps {
  ...
  onClose: () => void;
  onSaved: () => void;
  guide: BuyingGuide | null;
}

export default function GuideModal({ isOpen, onClose, onSaved, guide }: GuideModalProps) {
  const isEditing = Boolean(guide);

  const emptyForm = { id: "", advertiserId: "", title: "", readTime: "", summary: "", category: "", author: "", date: new Date().toISOString().split("T")[0] };
  ...
  useEffect(() => {
    if (guide) {
      setFormData({ id: String(guide.id), advertiserId: String(guide.advertiserId), title: guide.title, readTime: guide.readTime, summary: guide.summary, category: guide.category, author: guide.author, date: guide.date.split("T")[0] });
      setAdvSearch(String(guide.advertiserId));
    } else {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const payload = { ...formData, id: formData.id ? Number(formData.id) : undefined, advertiserId: Number(formData.advertiserId), date: new Date(formData.date).toISOString() };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/50 p-4 backdrop-blur-sm">
      <div className="relative w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl transition-all">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">{isEditing ? `Edit Guide (#${guide?.id})` : "Add Guide"}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              <label className="block text-xs font-medium text-gray-700">Guide Title *</label>
              <input type="text" required value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" />
            </div>
            <div className="col-span-2">
              <label className="block text-xs font-medium text-gray-700">Summary *</label>
              <textarea required rows={2} value={formData.summary} onChange={(e) => setFormData({ ...formData, summary: e.target.value })} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-700">Category</label>
              <input type="text" value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-700">Read Time</label>
              <input type="text" value={formData.readTime} onChange={(e) => setFormData({ ...formData, readTime: e.target.value })} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-700">Author</label>
              <input type="text" value={formData.author} onChange={(e) => setFormData({ ...formData, author: e.target.value })} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-700">Date</label>
              <input type="date" required value={formData.date} onChange={(e) => setFormData({ ...formData, date: e.target.value })} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" />
            </div>
          </div>
